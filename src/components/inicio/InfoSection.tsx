...
function InfoSection({ index, slider = false, imgSrc, imgTitle, gallery = [], title, description, button = true, customContent }: InfoSectionProps) {
    return (
        <section className="h-[50svh] md:min-h-[100svh] w-full relative">
            {
                slider ? <FadeImageSlider slides={gallery} /> :
                    <img className="size-full absolute object-cover" src={imgSrc} alt={imgTitle} />
            }
            <div className={clsx("xl:w-[20%] lg:w-[30%] md:w-[40%] w-full z-10 p-4 flex flex-col justify-center absolute text-white min-h-full md:min-h-[100svh] bg-[#0000003d] backdrop-blur-[1px] md:backdrop-blur-md shadow-section gap-10",
