...
function InfoSection({ index, slider = false, imgSrc, imgTitle, gallery = [], title, description, button = true, customContent }: InfoSectionProps) {
    return (
        <section className="h-[50dvh] md:min-h-[100dvh] w-full relative">
            {
                slider ? <FadeImageSlider slides={gallery} /> :
                    <img className="size-full absolute object-cover" src={imgSrc} alt={imgTitle} />
            }
            <div className={clsx("xl:w-[20%] lg:w-[30%] md:w-[40%] w-full z-10 p-4 flex flex-col justify-center absolute text-white min-h-full md:min-h-[100dvh] bg-[#0000004d] backdrop-blur-sm md:backdrop-blur-md shadow-section gap-10", index % 2 === 0 ? "md:right-[50px]" : "md:left-[50px]")}>
